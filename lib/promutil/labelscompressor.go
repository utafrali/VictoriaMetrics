...
func (lc *labelsCompressor) decompress(dst []prompb.Label, src []byte) []prompb.Label {
	...
	dst = lc.decompressFrom(dst, a.A)
	encoding.PutUint64s(a)
	return dst
}

func (lc *labelsCompressor) decompressFrom(dst []prompb.Label, src []uint64) []prompb.Label {
	for _, idx := range src {
...
type LabelsCompressor struct {
	...
	registry   map[uint64]time.Duration
...
func (lc *LabelsCompressor) Register(maxStaleness time.Duration) uint64 {
	id := lc.nextID.Add(1)

	lc.registryMu.Lock()
	if lc.registry == nil {
		lc.registry = make(map[uint64]time.Duration)
	}
	lc.registry[id] = maxStaleness
	max := lc.maxStaleness()
...
func (lc *LabelsCompressor) Unregister(id uint64) {
	lc.registryMu.Lock()
	delete(lc.registry, id)
	max := lc.maxStaleness()
	lc.registryMu.Unlock()
	lc.rotationInterval.Store(int64(max * 2))
}
